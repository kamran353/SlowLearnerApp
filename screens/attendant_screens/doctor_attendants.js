...
const attendants = ({ navigation }) => {
  ...
  useEffect(() => {
    AsyncStorage.getItem('User')
      .then((value) => {
        ...
        console.log(value)
        if(user.UserRole=="PA"){
            getMyAttendants(user.ReferenceUserId) 
        }else{
            getMyAttendants(user.UserId)
        }
  ...
  function getMyAttendants(doctorId) {

    axios.get(`${global.BaseUrl}GetDocAttendants?DoctorId=${doctorId}`).then((response) => {
  ...
  return (
    <View style={styles.container}>
      {MyPatients.length > 0 ?
          ...
          renderItem={({ item }) => (
            <CardView
              ...
              <View style={styles.buttonView}>
                ...
                <TouchableOpacity style={{ marginLeft: 20 }} onPress={() => navigation.navigate("AssignPatients", { AttendantId: item.UserId })}>
                  <Text style={styles.acceptTxt}>
                    Assign Patient
                  </Text>
                </TouchableOpacity>
        ...
        <TouchableOpacity
        onPress={() => navigation.navigate('RegisterAttendant')}
        activeOpacity={1}
