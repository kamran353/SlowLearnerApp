...
const assignPatients = ({ navigation,route }) => {
  ...
  useEffect(() => {
    AsyncStorage.getItem('User')
      .then((value) => {
        ...
        console.log(value)
        if(user.UserRole=="PA"){
            getMyPatients(user.ReferenceUserId) 
        }else{
  ...
  function AssignPatientToAttendant(id) {
    const attendantPatient = {   
        AttendantId : route.params.AttendantId,
        PatientId: id
        }
      axios.post(`${global.BaseUrl}AssignAttendantToPatient`, attendantPatient)
        .then(response => {
          ...
          renderItem={({ item }) => (
            <CardView
              ...
              <View style={styles.buttonView}>
               <TouchableOpacity style={{ marginLeft: 20 }} onPress={() => AssignPatientToAttendant(item.UserId)}>
